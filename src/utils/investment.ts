 ...
 */
export async function getInvestmentPrice(symbol: string, classId: string): Promise<{ price: number; mode: PriceMode }> {
    ...
    if (marketData) {
        return { price: marketData.price, mode };
    }

    // 스냅샷이 없는 경우 실시간 가격을 가져와서 반환 (Fall-back)
    const fallbackPrice = await fetchLivePrice(symbol);
    return { price: fallbackPrice || 0, mode: 'realtime' };
}
